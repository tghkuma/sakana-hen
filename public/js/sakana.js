...
//======================
/** フレームレート */
SakanaHen.prototype.frame_rate = 24;
/** 寿司移動時間(mS) */
...
/** 解答選択数 */
SakanaHen.prototype.max_anser_no = 3;
...
//		return false;
	...
	// 画像ファイルロード
	for (var i = 0; i < this.lst_images.length; i++) {
		var image = new Image();
		image.onload = function(event){self.onload(event)};
		image.onerror = function(event){self.onerror(event)};
...
//console.log("音声拡張子:"+ext);
	...
	if (ext){
		for (var i = 0; i < this.lst_audios.length; i++) {
			audio = new Audio();
...
SakanaHen.prototype.onload = function(event) {
	...
	var strMessage = "ロード成功:"+this.load_count+"/"+this.load_max_count+"["+event.target.getAttribute("src")+"]";
...
SakanaHen.prototype.onerror = function(event) {
	...
	var strMessage = "ロード失敗:"+this.load_count+"/"+this.load_max_count+"["+event.target.getAttribute("src")+"]";
	console.log(strMessage);
	this.ctx.font = "12px "+this.font_common;
	this.ctx.fillText(strMessage,0,20+(this.load_count*12));
}
...
SakanaHen.prototype.title = function() {
	...
	var strTitle = "『さかなへん』クイズ";
	var tm = this.ctx.measureText(strTitle);
	var posx = (this.canvas.width-tm.width)/2;
	var posy = (this.canvas.height-60)/2;
...
SakanaHen.prototype.startGame = function() {
	...
	// 問題生成
	//==============
	var sakana_length = this.lst_sakana_hen.length;

	// データ転記
	var arr_sakana_hen_tmp = [];
	for(var i=0; i < sakana_length; i++){
		arr_sakana_hen_tmp.push([this.lst_sakana_hen[i][0], this.lst_sakana_hen[i][1]]);
	}
	// シャッフル
	for(i=0; i < 100; i++){
		var item_no_0 = Math.floor(Math.random()*sakana_length);
		var item_no_1 = Math.floor(Math.random()*sakana_length);

		var item_tmp = [arr_sakana_hen_tmp[item_no_0][0], arr_sakana_hen_tmp[item_no_0][1]];
	...
	// 問題/回答生成
	this.arr_quiz = [];
	for(i=0; i < this.max_quiz_no; i++){
		var anser_no = Math.floor(Math.random()*this.max_anser_no);
		console.log("問題"+i+":"+anser_no);
		var selection = [];
		for(var sel_no=0; sel_no<this.max_anser_no; sel_no++){
			var quiz = arr_sakana_hen_tmp[i][0];
			var anser = arr_sakana_hen_tmp[i][1];
			// 当たり生成
			if(sel_no == anser_no){
				selection.push(anser);
			}
			// はずれ生成
			else{
				var sel_tmp;
...
//						console.log("あたり発見:"+anser);
	...
	//==============
	// タイマー値計算
	var interval_time = 1000/this.frame_rate;
	...
	// タイマー設定
	var self= this;
	this.timer_id = setInterval(function(){self.timer()}, 1000/this.frame_rate);
	...
	if (!this.isSmartPhone()){
		this.click_event_callee = function(event){self.click(event)};
		this.canvas.addEventListener('click', this.click_event_callee, false);
	}
	// スマホ
	else{
		this.click_event_callee = function(event){self.touchstart(event)};
...
* @param y Y座標
*/
SakanaHen.prototype.checkAnser = function(x, y) {
	...
	// 回答判定
	for(var i=0; i < this.max_anser_no;i++) {
		posy = 250+50*i;
		if (posy <= y && y < posy+50) {
			this.selecsion_click_no = i;
			console.log("選択="+this.selecsion_click_no);

			var item_quiz = this.arr_quiz[this.quiz_no];
			// あたり/はずれ処理
			this.setHit(this.selecsion_click_no == item_quiz.anser_no);
...
//	this.ctx.fillStyle = "rgb(255,255,0)";
	...
	// 寿司＋さかなへん
	//----------
	posx = this.canvas.width-(((this.canvas.width+120)/this.susi_move_count_org)*this.susi_move_count)+60;
	posy = 146;
	// 皿
	...
	// クイズ
	var item_quiz = this.arr_quiz[this.quiz_no];
	var strQuiz = item_quiz.quiz;
	if(this.debug){
		strQuiz += "("+item_quiz.anser+")";
	}
...
//console.log("x="+this.selecsion_click_no+","+i);
	...
	if(0 < this.hit_wait){
		...
		var img_ans = this.bHit ? this.img_ans_ok:this.img_ans_ng;
		posy = 120+(this.hit_wait/this.hit_wait_org*40);
	...
	//----------
	// 問題
	var strQuizNo = "問題:"+(this.quiz_no+1)+"/"+this.arr_quiz.length;
	this.ctx.font = "30px "+this.font_common;
	this.ctx.fillStyle = "rgb(0,0,0)";
	var tm = this.ctx.measureText(strQuizNo);
	var posx = this.canvas.width-tm.width;
	this.ctx.fillText(strQuizNo, posx, 30);

	// 正解数
	var strScore = "正解:"+this.hit_count;
...
SakanaHen.prototype.endGame = function() {
	...
	var strResult;
	var aResult;
	for (var i=0; i < this.result_info.length; i++){
		var item = this.result_info[i];
	...
	// 正解数
	var strScore = "正解:"+this.hit_count+"/"+this.arr_quiz.length;
