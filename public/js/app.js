...
class SakanaHen {
  ...
  startGame() {
    ...
    if (!this.isSmartPhone()) {
      this.onClickAnswerFunc = (event) => this.onClickAnswerFuncClick(event);
      this.canvas.addEventListener('click', this.onClickAnswerFunc, false);
      this.onMouseMoveAnswerFunc = (event) => this.onMouseMoveAnswer(event);
      this.canvas.addEventListener('mousemove', this.onMouseMoveAnswerFunc, false);
    }
    // スマホ
    else {
      this.onClickAnswerFunc = (event) => this.onClickAnswerFuncTouch(event);
